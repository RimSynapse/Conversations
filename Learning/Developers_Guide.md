...
> **⚠ Retired under the 0.10 "Motivated Dialogue" shift (Conversations#60 / #61).** `ChatTopicDef` and its XML catalogue have been **deleted**, and the colonist-side context keys (`ownedRoom`, `apparel`, `health`, `bondedAnimal`, `food`, `recipientRelationship`, `personalitySummary`, `residency`) have been **removed** from `ConversationContextResolver` — only the memory-tier keys and the prisoner-side warden keys remain. The sections below that describe them are kept for historical reference and no longer match the code. The replacement extension point is the per-pawn **talking-point agenda** — see the [Motivated Dialogue](Motivated_Dialogue) reference page for the point model and the full formation-rule catalogue (build spec: `docs/motivated-dialogue-agenda.md` in the repo); this guide will be rewritten against it as the pieces land. **RimSynapse - Psychology is now a hard dependency**; every line is voiced from the pawn's psychology profile. Also retired in step 4: the Harmony postfix on `Pawn_InteractionsTracker.TryInteractWith` (the vanilla ambient trigger and its initiation/response-chance gates), the environmental darkness/freezer driver, load-shedding, the pair-keyed pre-seed pool and event pre-staging — the pipeline, Harmony-surface and debug-action sections below that describe them are historical. Live generation survives only behind the debug force actions.

The two extension points a content modder *used to* reach for most were **[ChatTopicDef](#adding-conversation-topics-chattopicdef)** (pure XML, no code) and the **[context-key system](#context-keys)** (a small C# switch) — see the retirement note above. The rest of this guide documents the generation pipeline, the warden system, the Harmony surface, and the registered game tools for deeper integration.
