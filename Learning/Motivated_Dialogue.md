...
| 4 | **Bond shift** | any conversant | a notable trust change in Psychology `socialNetwork` (warmth axis returns with #72) | by magnitude | by magnitude | FirstHand | `Not(X)` — never said *to* the pawn it's about | "Something's off with Marco lately." | built (step 2; in-game validation queued on #59) |
