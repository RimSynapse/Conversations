...
namespace RimSynapse.Conversations.Tests
{
    ...
    public static class PromptComposerCases
    {
        ...
        public static IEnumerable<SynapseTestCase> All()
        {
            ...
            // Compose: the per-line speaker schema (#40). The composer must ask for {"speaker","text"}
            // objects and name both legal speakers — this is what unlocks three-way exchanges, and what
            // the old positional {"lines": ["a", "b"]} schema could not express.
            yield return new SynapseTestCase("PromptComposer_PerLineSpeakerSchema", () =>
            {
                var p = ThinPromptComposer.Compose("Ana", "Bex", "id-a", "id-b", Beat(), null);
                Assert.True(p.system.Contains("\"speaker\": \"Ana\""), "the JSON example tags the initiator");
                Assert.True(p.system.Contains("\"speaker\": \"Bex\""), "the JSON example tags the recipient");
                Assert.True(p.system.Contains("MUST be exactly one of: Ana, Bex"), "both speakers are named as the legal set");
                Assert.False(p.user.Contains("alternating and STARTING"), "the rigid-alternation instruction is gone");
                Assert.True(p.user.Contains("tagging each line with its speaker"), "the user turn asks for per-line tagging");
                return "per-line speaker schema present, rigid alternation dropped";
            });

            // Compose: the plain-speech register steer (#44) — the fix for colonists talking like analysts.
            yield return new SynapseTestCase("PromptComposer_PlainSpeechSteer", () =>
            {
                var p = ThinPromptComposer.Compose("Ana", "Bex", "id-a", "id-b", Beat(), null);
                Assert.True(p.system.Contains("NOT analysts"), "the anti-report framing is present");
                Assert.True(p.system.Contains("clinical or technical register"), "the register steer is present");
                Assert.True(p.system.Contains("'quantifiable'"), "the banned-word list is carried");
                Assert.True(p.system.Contains("UNLESS a speaker's own described voice is explicitly that way"),
                    "an authored clinical voice still wins over the steer");
                return "plain-speech steer present with the authored-voice exemption";
            });

            // Compose: the relationship note (#53) is optional and lands on the recipient's line. Defaulted
            // so the Prompt Lab's six-argument call keeps compiling.
            yield return new SynapseTestCase("PromptComposer_RelationshipNoteOptional", () =>
            {
                var bare = ThinPromptComposer.Compose("Ana", "Bex", "id-a", "id-b", Beat(), null);
                var noted = ThinPromptComposer.Compose("Ana", "Bex", "id-a", "id-b", Beat(), null,
                    " They are not equals: one belongs to the colony, the other is held by it.");
                Assert.False(bare.user.Contains("not equals"), "peers carry no relationship clause");
                Assert.True(noted.user.Contains("They are not equals"), "the clause reaches the prompt");
                Assert.True(noted.user.Contains("They respond: reacting to it. They are not equals"),
                    "the clause attaches directly to the recipient's stance line");
                return "relationship note optional and correctly placed";
            });

            // Identity: the cap must not truncate the sample line of speech a voiceProfile ends with (#44).
            yield return new SynapseTestCase("PromptComposer_VoiceProfileCapKeepsSampleLine", () =>
            {
                string voice = new string('x', 150) + " Might say: «Roof's patched.»";
                string id = IdentityComposer.Identity(voice, "Nomad", null);
                Assert.True(id.Contains("Might say: «Roof's patched.»"), "a 179-char voice keeps its sample line (220 cap, not 140)");

                string tooLong = new string('y', 400);
                Assert.True(IdentityComposer.Identity(tooLong, null, null).Contains("…"), "beyond the cap it is still elided");
                return "220-char cap preserves the sample line";
            });
        }
    }
}
