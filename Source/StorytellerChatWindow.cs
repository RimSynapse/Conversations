...
namespace RimSynapse.Chat
{
    ...
    public class StorytellerChatWindow : Window
    {
        ...
        public override void DoWindowContents(Rect inRect)
        {
            ...
            float totalHeight = CalculateScrollHeight(worldComp.chatHistory, scrollWidth);
            ...
            float curY = 0f;
            foreach (var msg in worldComp.chatHistory)
            {
        ...
        private void SubmitMessage(SynapseChatWorldComponent worldComp, string messageText)
        {
            ...
            int startIdx = Mathf.Max(0, worldComp.chatHistory.Count - 10);
            for (int i = startIdx; i < worldComp.chatHistory.Count; i++)
            {
                var msg = worldComp.chatHistory[i];
                apiMessages.Add(new ChatMessage
                {
